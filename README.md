...
**Nice to have, not blocking (2026-07-23, updated 2026-07-24):** input from Sean Carney or Sonny (Astroid lead dev), both already in the loop via the Members Only email thread. As of 2026-07-24 they've been asked directly which roadmap candidate above, per-group exclusion or URL-routing, sounds more likely to get used on real sites, their answer may reorder the list. Worth naming plainly: this is generic Joomla plugin code, nothing Astroid-specific, so any input would be a favour rather than a domain-expert review (self-testing disclosure up front, see "Read this before using it on a real site").
